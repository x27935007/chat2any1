...
一个论点要被认定为「心智模型」而非「随口一说」，必须通过四重验证：
...
### 验证2: 可泛化（通用性检查）
这个心智模型能够从具体事件中抽象出更一般的原则，而不是绑定在单一事件或场景里。

检查方法：
- 模型名称是否绑定特定事件？（如「直播还债逻辑」→ 绑定抖音直播这个具体事件）
- 去掉具体场景描述后，内核原则能否泛化到其他情境？
- 如果这个人遇到了一个他从未面对过的新问题，用这个模型能做出有价值的判断吗？

例：「欠债不是丑闻，还债才是新闻」——
- ❌ 原始表述绑定直播带货场景，离开抖音/2020年这个组合就失效
- ✅ 泛化内核：「底线是你愿意为之付出代价的东西，面对危机时亮出底线并坚守，是重建信任的最快方式」——可以应用于任何需要重建信任的情境

→ 泛化通过 → 进入验证4
→ 泛化失败 → 降级为「决策启发式案例」而非心智模型，不直接作为心智模型收录

### 验证3: 有生成力
用这个模型可以推断此人对新问题的可能立场，而不只是复述此人已说过的内容。
...
**验证结果判定**
- 4个验证全过 → **心智模型**（最高质量）
- 通过验证1+2+3，但验证4未过 → 降级为「有生成力的决策启发式」
- 只通过1-2个验证 → 降级为「决策启发式」
- 0个验证通过 → 不纳入（只是随口一说）
...
### 心智模型
- [ ] 每个模型都有至少2个不同领域的证据？
- [ ] **每个模型都能泛化？绑定了具体事件的（如「直播还债逻辑」）必须改名为更通用的内核表述**
- [ ] 模型数量在3-7个之间？（太少=太浅，太多=没提炼）
- [ ] 每个模型都有明确的应用场景和局限？
- [ ] 模型之间有张力但不矛盾？
- [ ] 能用这个模型推断此人对一个新问题的立场吗？（不能则生成力不足）
